...
class SimMatPooling(nn.Module):
    def __init__(self, config):
        ...
        in_channels = self.config['n_heads'] * 2


        self.cnn_1 = nn.Conv2d(in_channels=in_channels, out_channels=self.config['conv_channels_0'], kernel_size=(3, 3))
        self.pooling = nn.AdaptiveAvgPool2d((7, 7))
        self.cnn_2 = nn.Conv2d(in_channels=self.config['conv_channels_0'], out_channels=self.config['conv_channels_1'], kernel_size=(3, 3))
        self.cnn_3 = nn.Conv2d(in_channels=self.config['conv_channels_1'], out_channels=self.config['conv_channels_2'], kernel_size=(3, 3))
        self.cnn_4 = nn.Conv2d(in_channels=self.config['conv_channels_2'], out_channels=self.config['conv_channels_3'], kernel_size=(3, 3))
    ...
    def forward(self, sim_mat):
        out = F.leaky_relu(self.cnn_1(sim_mat), negative_slope=self.config['conv_l_relu_slope'], inplace=True)
        out = self.pooling(out)
        out = F.leaky_relu(self.cnn_2(out), negative_slope=self.config['conv_l_relu_slope'], inplace=True)
        out = F.leaky_relu(self.cnn_3(out), negative_slope=self.config['conv_l_relu_slope'], inplace=True)
        out = F.leaky_relu(self.cnn_4(out), negative_slope=self.config['conv_l_relu_slope'], inplace=True).squeeze()
