...
class FeedForwardNetwork(nn.Module):
    def __init__(self, config):
        ...
        hidden_size = self.config['max_num_nodes'] ** 2
        ffn_size = self.config['channel_ffn_size']
...
class MultiHeadAttention(nn.Module):
    def __init__(self, config):
        ...
        self.num_heads = num_heads = self.config['n_channel_transformer_heads']
        embedding_size = self.config['max_num_nodes'] ** 2

        self.att_size = embedding_size
        self.scale = embedding_size ** -0.5

        self.linear_q = nn.Linear(embedding_size, num_heads * embedding_size, bias=self.config['msa_bias'])
        self.linear_k = nn.Linear(embedding_size, num_heads * embedding_size, bias=self.config['msa_bias'])
...
class ChannelAlignment(nn.Module):
    def __init__(self, config):
        ...
        self.self_attention_norm = nn.LayerNorm(self.config['max_num_nodes'] ** 2)
        self.self_attention = MultiHeadAttention(self.config)
        self.self_attention_dropout = nn.Dropout(self.config['dropout'])

        self.ffn_norm = nn.LayerNorm(self.config['max_num_nodes'] ** 2)
    ...
    def forward(self, x, mask_ij):
        ...
        x = x.view(B, H, self.config['max_num_nodes'], self.config['max_num_nodes'])
