...
class NAGSLNet(torch.nn.Module):
    def __init__(self, config):
        ...
        if self.config['share_qk']:
            q = torch.nn.Linear(self.configconfig['embedding_size'], self.configconfig['embedding_size'] * self.configconfig['n_heads'], bias=self.configconfig['msa_bias'])
            k = torch.nn.Linear(self.configconfig['embedding_size'], self.configconfig['embedding_size'] * self.configconfig['n_heads'], bias=self.configconfig['msa_bias'])
        else:
            q = k = None
